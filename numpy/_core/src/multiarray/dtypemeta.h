...
typedef struct {
    ...
    */
    ...
    PyArrayDTypeMeta_IsInexact *is_inexact;
...
#define NPY_DT_CALL_is_inexact(dtype)  \
    (NPY_DT_SLOTS(dtype)->is_inexact != NULL ? \
        NPY_DT_SLOTS(dtype)->is_inexact(dtype) : -1)
#define NPY_DT_CALL_get_finfo(dtype, descr)  \
    (NPY_DT_SLOTS(dtype)->get_finfo != NULL ? \
        NPY_DT_SLOTS(dtype)->get_finfo(dtype, descr) : NULL)
